...
// Remove the duplicate mock - keep only this one
jest.mock("next/navigation", () => ({
  useRouter: jest.fn(),
}));
...
describe("SellerListingPage", () => {
  const mockRouterBack = jest.fn();
  const mockRouterPush = jest.fn(); // Add this

  beforeEach(() => {
    jest.clearAllMocks();
    
    // Mock both back AND push
    (useRouter as jest.Mock).mockReturnValue({ 
      back: mockRouterBack,
      push: mockRouterPush, // Add this
    });
  ...
  describe("Form Submission", () => {
    it("should submit successfully with valid data", async () => {
      ...
      // Changed from mockRouterBack to mockRouterPush
      await waitFor(() => expect(mockRouterPush).toHaveBeenCalledWith("/seller/me"));
    });
    ...
    it("should show loading state during submission", async () => {
      ...
      // Check that button shows "Creating..." text
      await waitFor(() => {
