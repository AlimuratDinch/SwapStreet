...
describe('SellerListingPage', () => {
  ...
  const fillForm = (includeImages = true, includeCategory = true) => {
    ...
    if (includeImages) {
      ...
      fireEvent.change(screen.getByLabelText(/images/i), { target: { files: [file] } });
    }
    
    if (includeCategory) {
      fireEvent.change(screen.getByLabelText(/^category/i), { target: { value: 'Shirts' } });
      fireEvent.change(screen.getByLabelText(/subcategory/i), { target: { value: 'T-shirts' } });
    }
  };
  ...
  describe('Validation Errors', () => {
    it('should show error for missing required fields', async () => {
      ...
      const form = screen.getByRole('button', { name: /create listing/i }).closest('form');
    ...
    it('should show error when no images uploaded', async () => {
      ...
      const form = screen.getByRole('button', { name: /create listing/i }).closest('form');
      fireEvent.submit(form!);
      expect(await screen.findByText('Please upload at least one image.')).toBeInTheDocument();
    });
    ...
    it('should show error for more than 5 images', async () => {
      ...
      expect(await screen.findByText('You can upload a maximum of 5 images.')).toBeInTheDocument();
    });
  });
  ...
  describe('Category Selection', () => {
    ...
    it('should reset subcategory when category changes', () => {
      ...
      fireEvent.change(screen.getByLabelText(/^category/i), { target: { value: 'Shirts' } });
      fireEvent.change(screen.getByLabelText(/subcategory/i), { target: { value: 'T-shirts' } });
      fireEvent.change(screen.getByLabelText(/^category/i), { target: { value: 'Pants' } });
